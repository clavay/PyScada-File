...
class Device:
    ...
    def __init__(self, device):
        ...
        self.device = device
        try:
            if hasattr(self.device.filedevice, 'instrument_handler') \
                    and self.device.filedevice.instrument_handler is not None \
                    and self.device.filedevice.instrument_handler.handler_path is not None:
                sys.path.append(self.device.filedevice.instrument_handler.handler_path)
                mod = __import__(self.device.filedevice.instrument_handler.handler_class, fromlist=['Handler'])
